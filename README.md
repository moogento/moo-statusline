...
- `⛁ 65k/155k(200k)` - Current context usage / compact threshold (effective window)
  - Compact threshold = effective window - 45K (matching Claude Code's default)
  - Parenthetical shows the effective window (`autoCompactWindow` if set, otherwise `context_window_size`)
...
This shows `⛁ 65k/455k(500k)` — compact triggers at 455K (500K - 45K), with 500K as the effective window.

The statusline reads settings in priority order: project `settings.local.json` > project `settings.json` > global `settings.local.json` > global `settings.json`.
...
5. **Calculates context usage**:
   ...
   - Compact threshold = effective window - 45K; effective window is `autoCompactWindow` if set, else `context_window_size`
   - Converts to k format (e.g., `65k/455k(500k)`)
